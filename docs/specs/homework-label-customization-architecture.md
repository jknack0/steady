...
The `displayLabels` map is keyed by `sortOrder` (matching the existing response key pattern) and contains the fully resolved label for each item. This is computed server-side so mobile needs zero config awareness.
...
**State management:** Add `homeworkLabels` to the page's form state (type `Record<string, string>`). Populate from `config.homeworkLabels` on load. On save, include it in the `PATCH /api/config/homework-labels` call (separate from the main PUT, or extend the main PUT -- extending the main PUT is simpler and follows the current save-all pattern).
...
1. In the collapsible header of each item (line ~1131 area where `typeConfig.label` is shown), show the **resolved label** instead of the hardcoded `typeConfig.label`.
...
In the `HomeworkRenderer` function (line ~630), replace the hardcoded type label:
